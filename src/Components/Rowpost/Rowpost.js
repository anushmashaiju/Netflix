...
const Rowpost = (props) => {
  ...
  const handleTrailers = (movie) => {
    axios.get(`https://api.themoviedb.org/3/movie/${movie.id}/videos?api_key=${api_key}&language=en-US`)
      .then((response) => {
        console.log("===response===", response.data.result[0].key);
      ...
      <div className='posters'>
        {Rowpost.map((movie) => {
          return (
            ...
            <img key={movie.id} onClick={()=>handleTrailers(movie)}
            className={props.isSmall === true ? "small-posters" : "myposters"} src={`${imageUrl + movie.backdrop_path}`} alt=""
